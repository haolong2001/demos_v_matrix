...
public:
  ...
  Eigen::ArrayXi migration_births(int index, const Eigen::ArrayXXi &age_matrix,
                                  const Eigen::ArrayXXi &migrat_mat);

  Eigen::ArrayXXi
  generateAgefromBirth(int index, const Eigen::ArrayXi &birthvec,
                       const std::vector<ArrayXXf> &disappear_mat);

  Eigen::ArrayXi calculateNewborns(int index, const ArrayXXi &femalebirthAge,
                                   int birth_start);
