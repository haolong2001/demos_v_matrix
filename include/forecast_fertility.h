...
std::vector<std::vector<std::vector<double>>> readFertilityMatrix(const std::string& filename);

// Helper function to map fertility rates based on age and year
float MapFertilityRate(int index, int year, int age, 
                      const std::vector<std::vector<std::vector<double>>>& fertility_rates);

// Helper function to print matrix preview and statistics
void printFertilityMatrixPreview(const std::vector<std::vector<std::vector<double>>>& matrix);
...
std::pair<VectorXi, VectorXi> calculateBirths(const MatrixXi& popu_mat, 
                    const std::vector<std::vector<std::vector<double>>>& fertility_rates,
                    int fertility_index); 
