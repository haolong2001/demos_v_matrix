...
namespace std {
template <typename T> unique_ptr<T> make_unique_array(size_t size) {
...
public:
  ...
  bool readPopuMat(
      const std::string &filename = "./data/bin/result_matrix_data.bin");
  bool
  readMorEigMat(const std::string &filename = "./data/bin/mig_disappear.bin");
  bool readFerMat(
      const std::string &filename = "./data/bin/AESFR_matrix_combine.bin");
  bool readDisEigMat(const std::string &filename = "./data/bin/disappear.bin");
  bool readImmiEigMat(
      const std::string &filename = "./data/bin/migration_in_alter.bin");
...
// the result is not good, we try update the fertility rate and mortality rate
// from Singstats chinese is done, we are doing test for the other ethnicities
