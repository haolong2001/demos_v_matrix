...
The script mirrors the logic from the notebook snippet: for a given target year
index we aggregate all ethnicity/gender groups (8 in total) and report counts
and percentages for each G-stage (G1..G5) crossed with ACR categories (A1..A3).

For each albumin scenario we use:
  * Baseline ACR case (albumin scenario 0)
  * Upper-bound ACR case (albumin scenario -1)
...
def build_table_for_year(year: int, max_age: int = 74) -> pd.DataFrame:
    ...
    # Use only the requested single year (not cumulative!)
    year_indices = [year_index]

    n_simu = stage_mats[0].shape[1]  # number of simulations (assuming 2nd axis = sim)
    ...
    for idx in range(8):
        # For single year, only slice at year_index
        stage_base = stage_mats[idx][0, :, :, year_indices]
        stage_upper = stage_mats[idx][-1, :, :, year_indices]
        age_base = age_mats[idx][0, :, :, year_indices]
        age_upper = age_mats[idx][-1, :, :, year_indices]
        acr_base = acr_mats[idx][0, :, :, year_indices]
        acr_upper = acr_mats[idx][-1, :, :, year_indices]

        def accumulate(stage_slice, age_slice, acr_slice, df_target):
            # flattening across [sim, person, single_year]
    ...
    # Take average by dividing by n_simu
    df_counts = df_counts / n_simu
    df_counts_upper = df_counts_upper / n_simu
    ...
    # Format numbers to 2 decimals for display (since averages may not be integers)
    df_final = (
        df_counts.round(2).astype(str)
        + " (" + df_perc.round(2).astype(str) + "%), "
        + df_counts_upper.round(2).astype(str)
    ...
    df_final["Total"] = (
        row_totals_base.round(2).astype(str)
        + " (" + row_perc_base.round(2).astype(str) + "%), "
        + row_totals_upper.round(2).astype(str)
        + " (" + row_perc_upper.round(2).astype(str) + "%)"
    )

    total_row = (
        col_totals_base.round(2).astype(str)
        + " (" + col_perc_base.round(2).astype(str) + "%), "
        + col_totals_upper.round(2).astype(str)
    ...
    df_final.loc["Total", "Total"] = (
        f"{grand_base:.2f} (100.0%), {grand_upper:.2f} (100.0%)"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate CKD stage-ACR table.")
    parser.add_argument(
        "--year",
        type=int,
        default=2022,
...
if __name__ == "__main__":
    main()
# remark , by times 2 we can have actual number 
